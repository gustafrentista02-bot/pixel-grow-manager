...
import {
  ...
  MapPinned, Globe, Instagram, Check, X, Video,
} from "lucide-react";
...
import {
  ...
  addNote, updateLead, deleteLead, logLeadEvent, type Lead, type LeadFile,
} from "@/lib/leads-api";
    ...
    <div className="space-y-5">
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <div className="mt-1 flex flex-wrap items-center gap-2">
              {lead.empresa && <span className="text-sm text-muted-foreground">{lead.empresa}</span>}
              <Badge variant="outline" className={stage?.badge}>{stage?.emoji} {stage?.label}</Badge>
              <Badge variant="outline" className={pot.badge}>Potencial {pot.label}</Badge>
            </div>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          <AlertDialog>
            <AlertDialogTrigger asChild>
              <Button variant="outline" size="sm" className="text-red-400"><Trash2 className="h-4 w-4" /></Button>
            </AlertDialogTrigger>
            <AlertDialogContent>
              <AlertDialogHeader>
      ...
      {lead.meet_link && (
        <Button asChild variant="outline" size="sm" className="text-violet-300">
          <a href={lead.meet_link} target="_blank" rel="noreferrer"><Video className="mr-1.5 h-4 w-4" /> Entrar no Meet</a>
        </Button>
      )}
      ...
      <Tabs defaultValue="geral">
        ...
        <TabsContent value="marketing" className="mt-4">
          <Card>
            <CardContent className="grid gap-4 pt-6 sm:grid-cols-2 lg:grid-cols-3">
              <div className="space-y-0.5">
                <p className="text-[11px] uppercase tracking-wide text-muted-foreground">Perfil no Google</p>
              ...
              <div className="space-y-0.5">
                <p className="text-[11px] uppercase tracking-wide text-muted-foreground">Possui Site</p>
                <YesNo v={lead.tem_site} />
              </div>
              <div className="space-y-0.5">
                <p className="text-[11px] uppercase tracking-wide text-muted-foreground">Faz Google Ads</p>
                <YesNo v={lead.faz_google_ads} />
              </div>
              <div className="space-y-0.5">
                <p className="text-[11px] uppercase tracking-wide text-muted-foreground">Faz Meta Ads</p>
                <YesNo v={lead.faz_meta_ads} />
              </div>
              <Field label="Instagram" value={lead.instagram} />
              <Field label="Área de atendimento" value={lead.area_atendimento} />
              <div className="space-y-0.5 sm:col-span-2 lg:col-span-3">
                <p className="text-[11px] uppercase tracking-wide text-muted-foreground">Como chegam clientes hoje</p>
        ...
        <TabsContent value="historico" className="mt-4 space-y-4">
          <Card>
            ...
            <CardContent className="space-y-2">
              <Textarea value={note} onChange={(e) => setNote(e.target.value)} rows={2} placeholder="Registrar interação..." />
                ...
                <ol className="relative space-y-4 border-l border-border pl-4">
                  {eventsQ.data.map((ev) => (
                    <li key={ev.id} className="relative">
                      ...
                      <p className="text-xs text-muted-foreground">{formatDateTime(ev.created_at)}{ev.autor_nome ? ` · ${ev.autor_nome}` : ""}</p>
